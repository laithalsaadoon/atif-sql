...
**Why not "skip when the output exists".** An existence gate cannot express
"my input changed", so it inverts this pipeline: ``cluster`` re-fits whenever
the embeddings move, while ``terms`` and ``community`` skip forever on the
strength of their own stale outputs. HDBSCAN re-mints ``cluster_id`` on every
fit, so the result is c-TF-IDF labels and communities keyed to a partition
that no longer exists. An mtime gate expresses the dependency; an existence
gate does not.
