...
DELIBERATE TWIN of :mod:`atif_duck.domain.session_id`, the same way
:mod:`atif_corpus.domain.agents` twins the converter's enum: the independence
contract forbids the two packages from importing each other, and both sides
must apply the SAME rule (the corpus writer before it writes, the registry
before it reads a corpus an older version wrote). Each package's tests read
the other's copy as source text and fail if the pattern or the limit drifts.
