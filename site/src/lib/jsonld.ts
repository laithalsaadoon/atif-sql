 ...
 * `import.meta.env`, so every URL and every node it emits is asserted in a unit test rather than only in
 * a validator run against a deployed page.
 *
 * Every URL is built through `siteUrl`, which resolves with `new URL()`. A base segment joined to a path
 * by string concatenation produces `//product/index.md` — a protocol-relative URL naming a *host* called
 * `product` — and the failure is silent: the value parses, resolves to nothing, and looks right in the
 ...
 * article's `isPartOf` cannot be resolved against the site node and the relationship is asserted without
 * being expressible. One `@graph` with `@id`-addressed nodes is what makes the reference join.
 *
 * **Why `TechArticle` and not `Article`.** `TechArticle` is schema.org's subtype for technical
 * documentation and it carries the two properties a docs page can honestly fill — `proficiencyLevel` and
 * `dependencies`. `Article` is shaped for editorial and news, where the expected properties are
 * `author`, `datePublished` and `publisher`; a docs page usually has no single author and no publication
 * date distinct from its last edit, so filling those to satisfy a validator invents facts. An empty
 * optional beats a fabricated value: a consumer treats an absent property as unknown and a present one
 * as asserted.
 */
 ...
 * default is to omit it. A `SearchAction` is a promise that a GET against the template returns results,
 * and a docs site whose search runs entirely in the browser — Pagefind, and every other client-side
 * index — has no such endpoint. Declaring one anyway ships a machine-readable claim that 404s on the
 * first consumer that follows it. Supply this only when a real route answers it.
 */
...
export interface PageDescription {
  ...
  /**
   * Optional, and omitted rather than substituted.
   *
   * `docsSchema()` makes `description` optional, so a page can genuinely lack one. Filling it from
   * the site description would ship a machine-readable claim about this page that came from another
   * page: a consumer reads an absent property as unknown and a present one as asserted, so an empty
   * optional beats a borrowed value.
   */
 ...
 * so a consumer reads "same work, other bytes, `text/markdown`, here" without knowing this site's `.md`
 * convention. The `<head>` `rel="alternate"` link says the same thing to a different reader; neither
