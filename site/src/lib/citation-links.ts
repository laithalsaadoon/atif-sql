...
/**
 * ccu citations as links: an mdast `inlineCode` visitor that turns every `` `path:LOC` `` in a
 * generated docs tree into either an intra-site link or a repo permalink, at build time.
 *
 * `inlineCode` is a declared key on `MdastPluginInstance`
 * (`satteri@0.10.5/dist/mdast/mdast-visitor.d.ts`), and so are `heading`, `tableRow`, and the
 ...
 * `inlineCode` node, so `` `src/a.rs:10` `` renders as
 * `<a href="…"><code>src/a.rs:10</code></a>`. Rewriting the text would take the grep-able path away
 * from the reader who wants to run `rg` on it, which is most of them.
 *
 ...
 * and miss the twin.
 *
 * ## Every visitor here is synchronous, and this plugin is registered as a FACTORY
 *
 * The antecedent lives on the returned plugin's closure, so it is only correct while one document's
 * walk owns that closure. Two things keep that true, and both are required:
 *
 * - Every visitor here is synchronous, so this plugin's own walk cannot suspend mid-document. The
 *   presence check is `execFileSync`, memoized at module scope, rather than an await.
 * - `astro.config.ts` pushes `() => citationLinks(...)`, a factory, which Sätteri resolves once per
 *   compile. That is not belt-and-braces: the mermaid visitor in the same array IS async, which makes
 *   the whole mdast pass async, and Astro's glob loader renders up to ten entries concurrently. A
 *   single shared instance would then have its antecedent reset by one document while another was
 *   suspended, and a correct citation would resolve against the wrong file.
 */
...
export interface CitationLinksOptions {
   ...
   * Resolve it once at build time — `git rev-parse HEAD` — and pass the result:
   *
   *   commit: execFileSync("git", ["rev-parse", "HEAD"], { encoding: "utf8" }).trim()
   *
   * In CI prefer the checked-out SHA the runner already exports over re-resolving `HEAD`, which on a
   * merge-commit checkout is not the commit under review.
   */
  ...
  /**
   * The tree-relative paths the site actually publishes as routes.
   *
   * The set is narrower than "every `.md` under the tree", and the difference is a live defect
   * class: `docs/CONTRACT.md` sits in the tree and is not published, so deriving publication from
   * the filesystem emits an intra-site link to `/contract/`, a route the build never wrote. The
   * sync manifest names exactly what was published, so it is the input. Omit it and every tree page
   * present on disk is treated as published.
   */
  readonly publishedTreePaths?: ReadonlySet<string> | undefined
  /**
   * Site base plus the route prefix the tree is published under: `/docs/code`, no trailing slash.
   * Whatever `sync-ccu.mjs --target` or `ccu-loader.ts`'s `routePrefix` put the pages behind.
   */
...
export interface CitationTarget {
  ...
  /** The route slug, when the tree publishes this path as a page. */
  readonly slug: string | undefined
}

/**
 * The extensions a citation path may carry, verbatim from ccu's own validator
 * (`comprehensive-codebase-understanding/templates/crosslink.py`).
 *
 ...
 *
 *   src/a.rs:42            a full citation, which also becomes the antecedent
 *   src/a.rs:42-58         a range
 *   src/a.rs               a bare path: not a citation, but it names the antecedent
 *   :42                    a shorthand, resolved against the antecedent
 *
 ...
 * task packets, `.repomix/` the flattened pack. Both are gitignored in a ccu repo, so a citation
 * into one resolves on the author's disk and nowhere else — which is what the presence gate catches.
 */
const isSkippedDirectory = (name: string): boolean => name.startsWith(".")

/**
 * Paths present at one commit, from one `git ls-tree`, keyed by repository and commit.
 *
 * At MODULE scope rather than on the plugin instance, because the plugin is registered as a factory and
 * so is constructed once per document: an instance-level memo would run `git ls-tree -r` per page. The
 * key is the pair the answer depends on, so a build that ever pinned two commits cannot read one
 * commit's tree as another's.
 */
...
const trackedAtCommit = (repoRoot: string, commit: string): ReadonlySet<string> => {
  ...
  let tracked = trackedByCommit.get(key)
  if (tracked === undefined) {
    ...
    trackedByCommit.set(key, tracked)
  }
  return tracked
}
 ...
 * (`astro@7.2.4/dist/content/utils.js:270-272`).
 *
 * On `[A-Za-z0-9_-]` lowercasing is exactly what `github-slugger` does. Off it the two diverge —
 * `foo.bar` slugs to `foobar` — so a segment outside the set yields no slug and the citation takes
 * the permalink branch instead of shipping a link to a route that does not exist.
 */
...
/** Every `.md` file under `root`, as tree-relative slash-separated paths. */
...
/*
 * The plugin type comes from `satteri` itself, which this package depends on directly. The node and
 * context types are then reached through the visitor's own signature rather than through `mdast`, which
 * is transitive here and so does not resolve from this package under pnpm's isolated layout.
 *
 * Deriving the plugin type from the config's `mdastPlugins` array instead does not typecheck: an entry
 * there is a UNION of a plugin definition, a factory, a nested array and the falsy skip values, and
 * `Extract<…, { name: string }>` keeps the factory arm too, because every function has a `name` of type
 * `string`. The narrowed type then carries none of the visitor keys.
 */
 ...
 *   whose first cell is `` `src/two.rs` `` licenses `` `:30` `` in a later cell.
...
export const citationLinks = (options: CitationLinksOptions): MdastPluginDefinition => {
  ...
  /** The tree's published pages, enumerated once: the exact set of routes an intra-site link can hit. */
  let published: ReadonlySet<string> | undefined
  const publishes = (treePath: string): boolean => {
    published ??= options.publishedTreePaths ?? new Set(markdownUnder(options.treeRoot))
    return published.has(treePath)
  }

  /** Whether a path is present at `commit`. Absence is what the loud failure reports. */
  ...
  return {
    ...
    inlineCode(node, ctx) {
      ...
      const target: CitationTarget = {
        ...
        treePath,
        slug: treePath === undefined || !publishes(treePath) ? undefined : slugFor(treePath)
      }

      // Two destinations, each checked against what its own link resolves against: a published tree
      // page against the tree, everything else against the commit.
