 ...
 *
 * Copy to `src/lib/agent-note.ts` and register `agentNotePlugin()` in the Sätteri `mdastPlugins`
 * array. Three values are the adopter's own: `AGENT_NOTE_LABEL`, the `docs-` class prefix — which the
 * site's CSS has to match — and the plugin's `name`. The directive name is the authored syntax, so
 * changing it changes every page.
 *
 ...
 *
 * The wording is the adopter's vocabulary. The constant is not: every authored block repeats this
 * string by hand, so a gate asserts it rather than trusting ninety pages to agree.
 */
export const AGENT_NOTE_LABEL = "For an agent"

/** The class the note is styled through. The `docs-` prefix is the adopter's; the CSS matches it. */
...
/*
 * The plugin type comes from `satteri` itself, which this package depends on directly.
 *
 * Deriving it from the config's `mdastPlugins` array instead does not typecheck: an entry in that array
 * is a UNION of a plugin definition, a factory, a nested array and the falsy skip values, and narrowing
 * it with `Extract<…, { name: string }>` keeps the factory arm too — every function has a `name` of type
 * `string` — so the narrowed type has none of the visitor keys.
 */
type DirectiveVisitor = NonNullable<MdastPluginDefinition["containerDirective"]>

/*
 * The node type is reached through the visitor's own return type rather than through `mdast`, which is a
 * transitive dependency here and so is not resolvable from this package under pnpm's isolated layout.
 */
 ...
 *
 * Registration order matters and is in the adopter's favor: Starlight *pushes* its own mdast plugins
