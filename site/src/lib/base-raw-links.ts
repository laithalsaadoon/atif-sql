 ...
 *
 * Copy to `src/lib/base-raw-links.ts` and add `baseRawLinks(base)` to `integrations` in
 * `astro.config.ts`, passing the same string the config's `base` holds. The integration `name` prefix
 * is the adopter's; nothing else here is site-specific.
 *
 * The defect this closes, measured on a site published under a base segment: 450 links across the
 * `.md` twins point at `/section/page/` rather than `/<base>/section/page/`, and every one is a 404
 * for the agent following it. The HTML is correct throughout, which is precisely why it goes
 * unnoticed — the two surfaces are built from different things. `starlight-base-path` rewrites the
 * rendered tree, while `starlight-md-txt` builds each raw route from the page's Markdown source, so a
 * link is correct on whichever surface its producer touched and wrong on the other.
 *
