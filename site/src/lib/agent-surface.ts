 ...
 *
 * `Head.astro` and `PageActions.astro` import it as `../lib/agent-surface.js`. Everything here is
 * either a vendor fact or a warranted convention, and each carries the evidence for itself below.
 *
 ...
 * produces `//atif-sql/index.md` — a protocol-relative URL naming a *host* called `atif-sql` — which
 * is silent: the href parses, resolves to nothing, and looks right in source.
 */

/**
 * The corpus name a prompt uses to tell an assistant what it is reading.
 *
 * Exported so a gate can assert the placeholder is gone.
 */
export const DOCS_NAME = "atif-sql"
