...
`atif-sql = "atif_cli.app:main"` (`packages/atif-cli/pyproject.toml:43`), installed with
...
(`README.md:43`); they are uv workspace members (`pyproject.toml:9`). `atif-converter` wraps
...
(`pyproject.toml:364`) declares a layer contract per member, an independence contract forbidding
...
| --- | --- | --- |
| Language | Python, `requires-python = ">=3.13"` | `packages/atif-cli/pyproject.toml:15` |
| Toolchain pin | mise, `python = "3.13"` | `mise.toml:32` |
| Packaging | uv workspace, `members = ["packages/*"]` | `pyproject.toml:9` |
| Build backend | `uv_build>=0.11.14,<0.12` | `packages/atif-cli/pyproject.toml:52` |
| CLI framework | `cyclopts>=4.10.2` | `packages/atif-cli/pyproject.toml:38` |
...
| Model access | `boto3>=1.40.0` for Bedrock | `packages/atif-models/pyproject.toml:20` |
...
| Lint and format | ruff, `select = ["ALL"]` | `pyproject.toml:55` |
| Architecture gate | import-linter contracts | `pyproject.toml:364` |
| Tests | pytest, `testpaths = ["packages/*/tests"]` | `pyproject.toml:298` |
