...
The `atif-sql` console script (`packages/atif-cli/pyproject.toml:43`) is the repo's public contract: one cyclopts router (`packages/atif-cli/src/atif_cli/app.py:52`) dispatches the ten subcommands below, and a root-level `--version` resolves the installed `atif-sql` distribution (`:60`).
