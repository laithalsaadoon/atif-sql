...
| No source file may change, vanish, or appear between the one read that parses and fingerprints it and the end of the audit; the converter and the audit consume that one read's records, so each file is opened twice (read, then re-hash), parsed once, hashed twice | Conversion | `packages/atif-converter/src/atif_converter/application/convert_and_audit.py:93-102`, checked once after enrichment at `packages/atif-converter/src/atif_converter/application/convert_and_audit.py`; the read is `packages/atif-converter/src/atif_converter/infrastructure/raw_records.py` (`load_session`); tests `packages/atif-converter/tests/test_snapshot_and_drift.py` (`TestConvertRefusesOnMidConversionWrite`, `TestSinglePass`) | raise `SourceMutatedDuringConversion`; the materialize pass records the session as failed and continues |
...
| `subagent_files_convertible` equals `subagent_files_found`: our converter discovers every side file itself, workflow-nested ones included, so nothing found is unconvertible; the retired `workflow_subagents_missed` value is never emitted | Application code | `packages/atif-converter/src/atif_converter/application/convert_and_audit.py:64`; test `packages/atif-converter/tests/test_convert_and_audit.py:127` |
