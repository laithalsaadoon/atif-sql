...
| --- | --- | --- | --- |
| A session path must end in `.jsonl` AND be an existing file | Conversion | `packages/atif-converter/src/atif_converter/infrastructure/harbor_adapter.py:163-165`; test `packages/atif-converter/tests/test_convert_and_audit.py:224` | raise `InvalidSessionInput`; CLI exits 64 (`packages/atif-cli/src/atif_cli/app.py:262-264`) |
| harbor's private `_convert_events_to_trajectory` must exist and be callable | Conversion | `packages/atif-converter/src/atif_converter/infrastructure/harbor_adapter.py:112-139`; tests `packages/atif-converter/tests/test_snapshot_and_drift.py:296` and `packages/atif-converter/tests/test_snapshot_and_drift.py:319` | raise `HarborPrivateApiMissing`; CLI exits 127 (`packages/atif-cli/src/atif_cli/errors.py:38`) |
| harbor must return a trajectory for the session | Conversion | `packages/atif-converter/src/atif_converter/infrastructure/harbor_adapter.py:187` | raise `EmptySessionError`; CLI exits 2 (`packages/atif-cli/src/atif_cli/app.py:266-267`) |
| The trajectory must pass harbor's `TrajectoryValidator` AFTER enrichment mutates `extra` | Conversion | `packages/atif-converter/src/atif_converter/infrastructure/harbor_adapter.py:97-102`, `packages/atif-converter/src/atif_converter/application/convert_and_audit.py:150-154`; test `packages/atif-converter/tests/test_enrichment.py:472` | errors collected onto `ConversionResult.validation_errors`; `convert` exits 65 when non-empty (`packages/atif-cli/src/atif_cli/app.py:303-304`) |
