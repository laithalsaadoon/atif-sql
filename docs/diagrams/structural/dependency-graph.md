...
(`pyproject.toml:8-9`); external nodes are first-order distributions taken from a member's own
`[project.dependencies]`, never from the installed transitive closure.

The internal direction is enforced rather than conventional. `pyproject.toml:364` declares
...
those contracts fix the shape drawn here: the `independence` contract at `pyproject.toml:416-419`
...
| --- | --- | --- |
| atif-cli to atif-analytics | 1 | atif-cli is the composition root; it is absent from both restrictive contracts (`pyproject.toml:419`, `:426`) |
| atif-cli to atif-converter | 2 | as above; declared `packages/atif-cli/pyproject.toml:34` |
| atif-cli to atif-corpus | 2 | as above; declared `packages/atif-cli/pyproject.toml:35` |
| atif-cli to atif-duck | 2 | as above; declared `packages/atif-cli/pyproject.toml:36` |
| atif-cli to atif-embed | 1 | as above; declared `packages/atif-cli/pyproject.toml:37` |
| atif-analytics to atif-models | 7 | the `forbidden` contract's single permitted edge (`pyproject.toml:421-425`); declared `packages/atif-analytics/pyproject.toml:24` |
...
`[project.dependencies]` list omits atif-models accordingly (`packages/atif-cli/pyproject.toml:33-40`).
...
(`packages/atif-cli/pyproject.toml:33-37`) because `[tool.uv.sources]` at `:59-64` is a local source an
...
| pydantic | 7 | atif-models | `packages/atif-models/pyproject.toml:22` | `packages/atif-models/src/atif_models/infrastructure/openai_bedrock.py:41` |
...
| cyclopts | 2 | atif-cli | `packages/atif-cli/pyproject.toml:38` | `packages/atif-cli/src/atif_cli/app.py:35` |
...
- **loguru is universal.** All seven members declare it — `packages/atif-analytics/pyproject.toml:28`,
  `packages/atif-cli/pyproject.toml:39`, `packages/atif-converter/pyproject.toml:24`,
  `packages/atif-corpus/pyproject.toml:19`, `packages/atif-duck/pyproject.toml:21`,
  `packages/atif-embed/pyproject.toml:23`, `packages/atif-models/pyproject.toml:21` — and the edge is
...
at `uv.lock:1077-1102` lists `fastapi` (`:1082`), `supabase` (`:1097`), and `uvicorn` (`:1101`), so all
...
console script, `atif-sql = "atif_cli.app:main"` (`packages/atif-cli/pyproject.toml:43`), over a set of
libraries.
